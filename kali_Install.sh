...
mscode(){
    ...
    wget -qO- https://packages.microsoft.com/keys/microsoft.asc | $STD sudo gpg --dearmor > packages.microsoft.gpg
    $STD sudo install -D -o root -g root -m 644 packages.microsoft.gpg /etc/apt/keyrings/packages.microsoft.gpg
    echo "deb [arch=amd64,arm64,armhf signed-by=/etc/apt/keyrings/packages.microsoft.gpg] https://packages.microsoft.com/repos/code stable main" | $STD sudo tee /etc/apt/sources.list.d/vscode.list > /dev/null
